...
category_to_name() {
  ...
  case "$raw" in
    ...
    backoffice) echo "Back Office" ;;
    frontoffice) echo "Front Office" ;;
    core) echo "Core" ;;
    installer) echo "Installer" ;;
    webservices) echo "Web Services" ;;
    tests) echo "Tests" ;;
    localization) echo "Localization" ;;
    merge) echo "Merge" ;;
    projectmanagement) echo "Project management" ;;
    *) echo "Other" ;;
  esac
}
...
if [[ -z "$PR_NUMBERS" ]]; then
  echo "::warning::No merged PRs found between ${PREVIOUS_REF} and ${TARGET_REF}."
  PRS_JSON="[]"
  PR_COUNT=0
else
  ...
  for number in $PR_NUMBERS; do
    ...
    if [[ -n "$pr_info" ]]; then
      ...
      echo "::warning::Could not fetch PR #${number}, skipping." >&2
    fi
  done
...
while read -r pr; do
  ...
  echo "::notice::Processing PR ${idx}/${PR_COUNT} (#${number})..." >&2
  ...
  reasons=()
  [[ "$category_name" == "Other" || "$type_name" == "Other" ]] && reasons+=("Invalid or missing Type?/Category? in PR body")
...
if [[ -s "$ERRORS_FILE" ]]; then
  ...
  echo "---" >&2
  echo "::error::${ERROR_COUNT} PR(s) with errors. Files were not updated." >&2
  echo "" >&2
  echo "PRs in error:" >&2
...
for login in $PR_AUTHOR_LOGINS; do
  if echo "$BLACKLISTED_LOGINS" | tr ',' '\n' | grep -qixF "$login"; then
    echo "::notice::Skipping blacklisted account @${login}." >&2
  else
...
for login in $NEW_CONTRIBUTOR_LOGINS; do
  ...
  if [[ -n "$real_name" ]]; then
    ...
    echo "::warning::Could not resolve real name for @${login}, using username." >&2
